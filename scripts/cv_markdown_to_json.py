...
def parse_positions(positions_dir):
    """Parse positions from the _positions directory."""
    positions = []
    
    if not os.path.exists(positions_dir):
        return positions
    
    for positions_file in sorted(glob.glob(os.path.join(positions_dir, "*.md"))):
        ...
        if front_matter_match:
            ...
            positions.append(positions_entry)
...
def create_cv_json(md_file, config_file, repo_root, output_file):
    ...
    # Add positions
    cv_json["positions"] = parse_positions(os.path.join(repo_root, "_positions"))
